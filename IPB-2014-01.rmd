...
pd <- merge(ISAmtbls["assay.files"][[assaynr]], samplefactorvalues, by=Risa:::isatab.syntax$sample.name)
...
sdThresh <- 1.0 ## Filter low-standard deviation rows for plot
...
d <- phenoData(mtblsSet)[,c("Factor Value[Treatment]", "Factor Value[Sampling time]")]
...
injection <- as.numeric(sub(".*_([0-9]+).mzData", "\\1", filepaths(mtblsSet),perl=TRUE))
samplecolor <- rainbow(length(injection))[rank(injection)]
plotPcs(pca.result, type="loadings", main="PCA by injection number", col=samplecolor)
...
## Diffreport
```{r diffreport}
dr <- diffreport(mtblsSet, sortpval=FALSE, filebase="mtblsdiffreport", eicmax=20 )
cspl <- getPeaklist(an)
...
write.table(maf_character, 
            file="a_ipb-2014-01_metabolite profiling_mass spectrometry positive_v2_maf.tsv",
            row.names=FALSE, col.names=all.colnames, 
            quote=TRUE, sep="\t", na="\"\"")

ISAmtbls <- updateAssayMetadata(ISAmtbls, a.filename,
             "Metabolite Assignment File",
             "a_ipb-2014-01_metabolite profiling_mass spectrometry positive_v2_maf.tsv")
