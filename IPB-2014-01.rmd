...
mtblsSets <- sapply(c("microarray", "negative", "positive"), function(x) NULL)
...
for (assaynr in c(2,3)) {
  ...
  mtblsSets[[assaynr]] <- processAssayXcmsSet(ISAmtbls, a.filename,
...
#save(mtblsSets, file="mtblsSets.Rdata")
#load("mtblsSets.Rdata")
...
for (assaynr in c(2,3)) {
  ...
  sampclass(mtblsSets[[assaynr]]) <- as.factor(do.call(paste, xcmsSet.pd))
  phenoData(mtblsSets[[assaynr]]) <- xcmsSet.pd  
}
...
```{r retcor}
for (assaynr in c(2,3)) {
  mtblsSets[[assaynr]] <- group(mtblsSets[[assaynr]], minfrac=1, bw=4)
  mtblsSets[[assaynr]]
  mtblsSets[[assaynr]] <- retcor(mtblsSets[[assaynr]], plottype="mdevden")
  mtblsSets[[assaynr]] <- group(mtblsSets[[assaynr]], minfrac=1, bw=2)
  mtblsSets[[assaynr]]
  retcor(mtblsSets[[assaynr]], plottype="mdevden")
}
...
```{r QCintensity}
for (assaynr in c(2,3)) {
boxplot(groupval(mtblsSets[[assaynr]], value="into")+1, 
        col=as.numeric(sampclass(mtblsSets[[assaynr]]))+1, 
        log="y", las=2)
}
``` 
...
for (assaynr in c(2,3)) {
  mtblsSets[[assaynr]] <- fillPeaks(mtblsSets[[assaynr]], nSlaves=nSlaves)
}
...
for (assaynr in c(2,3)) {
  plotQC(mtblsSets[[assaynr]])
}
  
for (assaynr in c(2,3)) {
  sdThresh <- 1.5 ## Filter low-standard deviation rows for plot
  data <- log(groupval(mtblsSets[[assaynr]], value="into")+1)

  pca.result <- pca(data, nPcs=3)
  plotPcs(pca.result, type="loadings", 
          col=as.numeric(sampclass(mtblsSets[[assaynr]]))+1)

  d <- phenoData(mtblsSets[[assaynr]])[,c("Factor Value[Treatment]", 
  ...
  plotPcs(pca.result, type="loadings", main="PCA by genotype",
          col=phenoData(mtblsSets[[assaynr]])[,"Factor Value[Genotype]"])

  injection <- as.numeric(sub(".*_([0-9]+).mzData", "\\1", 
                              filepaths(mtblsSets[[assaynr]]),perl=TRUE))
...
ans <- sapply(c("microarray", "negative", "positive"), function(x) NULL)
for (assaynr in c(2,3)) {
  ans[[assaynr]] <- xsAnnotate(mtblsSets[[assaynr]],
                               sample=seq(1,length(sampnames(mtblsSets[[assaynr]]))),
                               nSlaves=nSlaves)
  
  ans[[assaynr]] <- groupFWHM(ans[[assaynr]])
  ans[[assaynr]] <- findIsotopes(ans[[assaynr]])  # optional but recommended.
  ans[[assaynr]] <- groupCorr(ans[[assaynr]],
                              graphMethod="lpc",
                              calcIso = TRUE,
                              calcCiS = TRUE,
                              calcCaS = TRUE,
  ...
  ans[[assaynr]] <- findAdducts(ans[[assaynr]],
                                rules=rs@rules,
                                polarity="positive")
  }
...
## Diffreport
```{r diffreport}
annotatedDiffreports <- sapply(c("microarray", "negative", "positive"), function(x) NULL)
for (assaynr in c(2,3)) {

dr <- diffreport(mtblsSets[[assaynr]], sortpval=FALSE, 
                  filebase=paste("mtblsdiffreport", assaynr, sep="-"), eicmax=0 )
cspl <- getPeaklist(ans[[assaynr]])

annotatedDiffreports[[assaynr]] <- cbind(dr, cspl)
}
...
pl <- annotatedDiffreports[[assaynr]] 

charge <- sapply(ans[[assaynr]]@isotopes, function(x) {
  ifelse( length(x) > 0, x$charge, NA) 
})
abundance <- groupval(ans[[assaynr]]@xcmsSet, value="into")
